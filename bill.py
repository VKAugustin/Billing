...
def additm():
    ...
    if item.get() != '':
        ...
        reset_values()  # Call the reset_values function to reset the values
    else:
...
def update_rate(event):
    selected_product = item.get()
    if selected_product in products_list:
        Rate.set(products_list[selected_product])

def gbill():
    if c_name.get() == "" or c_phone.get() == "":
        messagebox.showerror("Error", "Please provide customer details before generating the bill.")
    else:
...
def welcome():
    ...
    textarea.insert(END, "\t  Welcome Vidushan Retail")
...
title = Label(root, pady=2, text="Vidushan Retail", bd=12, bg=bg_color, fg='white', font=('times new roman', 25, 'bold'), relief=GROOVE, justify=CENTER)
title.pack(fill=X)
...
# Use Combobox for the dropdown menu
itm_txt = ttk.Combobox(F2, width=20, textvariable=item, values=list(products_list.keys()), font='arial 15 bold', state="readonly")
itm_txt.grid(row=0, column=1, padx=10, pady=20)
# Bind the function to the <FocusOut> event of the combo box
itm_txt.bind("<FocusOut>", update_rate)
